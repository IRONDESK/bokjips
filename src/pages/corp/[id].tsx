      ...
      {!isValidating && companyData ? (
        <Container isSplited={isSplited}>
          <SideOne>
            ...
            />
            <UpdateAtText>이 회사의 복지 정보는 {dateConvert(companyData?.updateAt)} 수정되었습니다.</UpdateAtText>
            {roleData?.roles === "ROLE_ADMIN" && <EditButtons companyId={corpId as string} />}
          </SideOne>
...
export const getServerSideProps: GetServerSideProps = async (context) => {
  ...
  const [companyData] = await Promise.all([swrFetcher(`${ServerURL}/${id}`, "")]);
